...
import { getCollectibles, getTokens, getCurrentProvider, isHtmlcoinAddressShow } from '../ducks/metamask/metamask';
import {
  TRANSACTION_TYPES,
...
export function useTransactionDisplayData(transactionGroup) {
  ...
  const isHtmlcoinAddressShowChk = useSelector(isHtmlcoinAddressShow);
  ...
  if (signatureTypes.includes(type)) {
    ...
    title = t('receive');
    prefix = '';
    subtitle = isHtmlcoinAddressShowChk ? t('fromAddress', [shortenAddress(getHtmlcoinAddressFromHex(senderAddress, chainId))]) : t('fromAddress', [shortenAddress(senderAddress)]);
  } else if (
    ...
    recipientAddress = getTokenAddressParam(tokenData);
    subtitle = isHtmlcoinAddressShowChk ? t('toAddress', [shortenAddress(getHtmlcoinAddressFromHex(recipientAddress, chainId))]) : t('toAddress', [shortenAddress(recipientAddress)]);
